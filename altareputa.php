<?php
    ...
    if(isset($_POST['nombre']) and !empty($_POST['nombre'])){
    	...
    	$consulta1="SELECT count(*) from reputacion where id_nombre = '$original'";
    	$query1=mysqli_query($conexi,$consulta1);
    	$cuenta=mysqli_fetch_array($query1);
    	if($cuenta['count(*)'] > 0){
    		header("location:agregarreputa.php?msj=Ya existe una reputacion con ese nombre");
            exit();
    	}
    }
    $ok=false;
    ...
    if((isset($_POST['nombre']) and !empty($_POST['nombre']) and  isset($_POST['min']) and !empty($_POST['min']) and  isset($_POST['max']) and !empty($_POST['max'])) or ($ok)){
        ...
        if($minimo > $maximo){
            ...
            header("location:agregarreputa.php?error=Asegurese que el puntaje minimo sea menor al puntaje maximo"); exit(); //}
        }
        if($minimo == $maximo){
            header("location:agregarreputa.php?igual=Los puntajes minimo y maximo no deben ser iguales");
            exit();
        }
        $borro="UPDATE reputacion set estado = 1 where estado = 0 and puntaje_inicial BETWEEN $minimo AND $maximo and puntaje_final BETWEEN $minimo AND $maximo";
        $borro2="UPDATE reputacion set estado = 1 where estado = 0 and puntaje_inicial BETWEEN $minimo AND $maximo and extremo = 0 and puntaje_final is null";
        //echo("hola");exit();
        ...
        $insertar="insert into reputacion  (`id_nombre`, `puntaje_inicial`, `puntaje_final`, `estado`, `extremo`)  values  ('$original', '$minimo', '$maximo', '0', 0)";
...
<?php
        ...
        //exit();
    	$minimo=$_POST['min'];
    	$borro="UPDATE reputacion set estado = 1 where estado = 0 and puntaje_inicial = $minimo and extremo = 0 and puntaje_final is null";
    	$hacer=mysqli_query($conexi,$borro);
    	$insertar="insert into reputacion  (`id_nombre`, `puntaje_inicial`, `estado`, `extremo`)  values  ('$original', '$minimo', '0', 0)";
    	$inserto=mysqli_query($conexi,$insertar);
    	$lista="SELECT * from reputacion where estado = 0 order by puntaje_inicial,puntaje_final";
        //echo($lista);exit();
    	$hago=mysqli_query($conexi,$lista);
...
$robert=mysqli_query($conexi,$juan);
if($prev == true){
    $consulta29="SELECT * from reputacion where id_nombre = '$original'";
    $query29=mysqli_query($conexi,$consulta29);
    $w=mysqli_fetch_array($query29);
    $consulta30="UPDATE reputacion set puntaje_inicial = $minimo where id_nombre = '$original'";
    $query30=mysqli_query($conexi,$consulta30);
    if ($opcional1 == $minimo){
    $puntajesito=$w['puntaje_inicial'] + 1;
...
$hago1=mysqli_query($conexi,$lista1);
        ...
        while ($a = mysqli_fetch_array($hago1)){
            ...
            if ($reputacion == $original){
                if (($ant_ide != "") and ($ant_inicial != "") and ($ant_extremo != "")){
                    if(($ant_inicial == $puntomin) and ($ant_extremo == 1) and ($a['puntaje_inicial'] == $ant_inicial)){
                        $puntitos=$a['puntaje_inicial'] -1;
                        $consulta35="UPDATE reputacion set puntaje_inicial = $puntitos where id_nombre = '$ant_ide'";
                        $query35=mysqli_query($conexi,$consulta35);
                        $consulta36="UPDATE reputacion set extremo = 0 where id_nombre = '$original'";
                        $query36=mysqli_query($conexi,$consulta36);
                    }
               }   
            }
            $ant_ide=$a['id_nombre'];
...
header("location:repuexitosa.php");
